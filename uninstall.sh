...
if [[ -d /run/systemd/system ]]; then
    systemctl disable nvcurve-autoload.service lpm-tune.service lpm-intel-uv.service lpm-intel-uv-daemon.service 2>/dev/null || true
else
...
rm -f "$PREFIX/bin/legion-power-manager" "$PREFIX/bin/nvcurve" "$PREFIX/bin/lpm-gamemode" "$PREFIX/bin/lpm-intel-uv" \
      /etc/init.d/lpm-tune /etc/init.d/lpm-intel-uv /etc/init.d/lpm-intel-uv-daemon {/lib64,/usr/lib64,/lib,/usr/lib}/elogind/system-sleep/50-lpm-intel-uv \
...
rm -f "$UNITDIR/nvcurve-autoload.service" "$UNITDIR/lpm-tune.service" "$UNITDIR/lpm-intel-uv.service" "$UNITDIR/lpm-intel-uv-daemon.service"
